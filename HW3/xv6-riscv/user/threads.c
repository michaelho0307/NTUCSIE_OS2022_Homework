...
void dispatch(void){
    if(current_thread->buf_set)
    {
        ...
        thrdresume(current_thread->thrdstop_context_id); //some problems
        //printf("3\n");
    }
    else // init
    {
...
void schedule(void){
    ...
    else if (current_thread->is_yield == 1){
        current_thread = current_thread -> next;
        roundrobincounter = 0;
    }
    else if (roundrobincounter >= 3){ //times up
        current_thread = current_thread -> next;
        roundrobincounter = 0;
    }
    roundrobincounter++;
    ...
    if (firstthread == 1){
        firstthread = 0;
        struct thread *toexecute = current_thread;
        int min_execute_time = current_thread->remain_execution_time;
        int minID = current_thread->ID;
        struct thread *head = current_thread;
        while(current_thread->next != head){
            current_thread = current_thread->next;
            if (current_thread->remain_execution_time < min_execute_time){
                toexecute = current_thread;
                min_execute_time = current_thread->remain_execution_time;
                minID = current_thread->ID;
            }
            else if (current_thread->remain_execution_time == min_execute_time && current_thread->ID < minID){
                toexecute = current_thread;
                min_execute_time = current_thread->remain_execution_time;
                minID = current_thread->ID;
            }
        }
        current_thread = toexecute;
    }
    /*else*/ if (firstthread == 0){
        if (current_thread->is_exited == 1){
            struct thread *oldthread = current_thread;
            current_thread = current_thread->next;
            struct thread *toexecute = current_thread;
            int min_execute_time = current_thread->remain_execution_time;
            int minID = current_thread->ID;
            while(current_thread->next != oldthread){
                current_thread = current_thread->next;
                if (current_thread->remain_execution_time < min_execute_time){
                    toexecute = current_thread;
                    min_execute_time = current_thread->remain_execution_time;
