...
bmap(struct inode *ip, uint bn)
{
  ...
  if (bn < NdoubleINDIRECT1){
    ...
    if((addr = a[firstlevel]) == 0){
      a[firstlevel] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
    bp_2 = bread(ip->dev, addr);
    a_2 = (uint*)bp_2->data;
    if((addr = a_2[secondlevel]) == 0){
      a_2[secondlevel] = addr = balloc(ip->dev);
      log_write(bp_2);
    }
    brelse(bp_2);
    return addr;
  ...
  while((path = skipelem(path, name)) != 0){
    ...
    if (ip->type == T_SYMLINK){
      ...
      //if (recur_count >= 11){
        ...
        //return 0;
      //}
      //memset(target_path,0,0);
      //readi(ip,0,(uint64)target_path,0,MAXPATH);
      //printf("original_namex_target:%s\n",target_path);
      //strcat(target_path,"/");
      ...
      //}
      //printf("old_namex_name:%s\n",name);
      //printf("old_namex_path:%s\n",path);

      //memset(path,0,strlen(path));
      //strncpy(path,target_path,strlen(target_path)+1);
      //strcat(path,"\0");
      //strncpy(path,target_path,strlen(target_path));
      //char *end = "\0";
      //strncpy(path,end,1);
      //printf("new_namex_path:%s\n",path);
      //iunlockput(ip);
      //ip = iget(ROOTDEV,ROOTINO);
      //ilock(ip);
      //continue;
...
namei(char *path)
{
  ...
  //printf("start\n");
  //struct inode* newway = namex(path, 0, name);
  //printf("end\n");
  //return newway;
